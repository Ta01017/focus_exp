...
run_rd(){ local c="${REDIFFUSE_CKPT:-}"; [[ -f "$c" ]]||{ skip 'REDIFFUSE_CKPT missing'; return; }; "$REDIFFUSE_PYTHON" "$ROOT/baselines/ReDiffuse/prepare_official_bytecode.py"; "$REDIFFUSE_PYTHON" "$ROOT/baselines/ReDiffuse/infer_metadata.py" --metadata "$METADATA" --output-dir "$OUTPUT_ROOT/ReDiffuse" --checkpoint "$c" --device cuda:0 --sampling-steps "$SAMPLING_STEPS" --start-index "$START_INDEX" --max-samples "$MAX_SAMPLES" --overwrite "$OVERWRITE"; }
