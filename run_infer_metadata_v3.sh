...
run_swin(){ local c="${SWINFUSION_CKPT:-}"; [[ -f "$c" ]]||{ skip 'SWINFUSION_CKPT missing'; return; }; "$PYTHON" "$ROOT/baselines/SwinFusion/infer_metadata.py" --metadata "$METADATA" --output-dir "$OUTPUT_ROOT/SwinFusion" --checkpoint "$c" --checkpoint-mode "$SWINFUSION_CHECKPOINT_MODE" --device cuda:0 --start-index "$START_INDEX" --max-samples "$MAX_SAMPLES" --overwrite "$OVERWRITE"; }
run_fd(){ local c="${FUSIONDIFF_CKPT:-}"; [[ -f "$c" ]]||{ skip 'FUSIONDIFF_CKPT missing'; return; }; "$PYTHON" "$ROOT/baselines/ImageFusion/FusionDiff/infer_metadata.py" --metadata "$METADATA" --output-dir "$OUTPUT_ROOT/FusionDiff" --checkpoint "$c" --device cuda:0 --sampling-steps "$SAMPLING_STEPS" --start-index "$START_INDEX" --max-samples "$MAX_SAMPLES" --overwrite "$OVERWRITE"; }
run_rd(){ local c="${REDIFFUSE_CKPT:-$ROOT/baselines/ReDiffuse/weights/model.pt}"; [[ -f "$c" ]]||{ skip 'REDIFFUSE_CKPT missing'; return; }; "$REDIFFUSE_PYTHON" "$ROOT/baselines/ReDiffuse/prepare_official_bytecode.py"; "$REDIFFUSE_PYTHON" "$ROOT/baselines/ReDiffuse/infer_metadata.py" --metadata "$METADATA" --output-dir "$OUTPUT_ROOT/ReDiffuse" --checkpoint "$c" --checkpoint-mode "$REDIFFUSE_CHECKPOINT_MODE" --device cuda:0 --sampling-steps "$SAMPLING_STEPS" --start-index "$START_INDEX" --max-samples "$MAX_SAMPLES" --overwrite "$OVERWRITE"; }
