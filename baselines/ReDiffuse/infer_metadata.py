...
def main():
    ...
    p.add_argument('--sampling-steps',type=int,default=2000); p.add_argument('--seed',type=int,default=0); p.add_argument('--config',default=str(ROOT/'config.json')); args=p.parse_args()
    ...
    if args.sampling_steps<1: raise ValueError('--sampling-steps must be positive')
    ...
    diffusion=GaussianDiffusion(args.sampling_steps,config['diffusion_model']['beta_schedule_type']); meta,items=load_metadata(args.metadata,args.start_index,args.max_samples)
    ...
    write_run_files(out,records,vars(args)|{'metadata':str(meta),'checkpoint_loaded':str(ckpt.resolve()),'actual_sampling_steps':args.sampling_steps})
