...
def main():
    ...
    p.add_argument('--size-policy',choices=('error','resize_b_to_a','center_crop_common'),default='error'); p.add_argument('--sampling-steps',type=int,default=2000); p.add_argument('--seed',type=int,default=0)
    ...
    diffusion=GaussianDiffusion(args.sampling_steps,config['diffusion_model']['beta_schedule_type'])
    ...
    write_run_files(out,records,vars(args)|{'metadata':str(meta),'checkpoint_loaded':str(ckpt),'actual_sampling_steps':args.sampling_steps})
